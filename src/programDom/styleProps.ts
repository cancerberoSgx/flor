...
export class StylePropsImpl< T extends StyleProps = StyleProps> extends AttrsImpl<Partial<T>> implements Partial<StyleProps> {
  ...
  protected calculatedWidth() {
    if (this._data.width && this._data.width > -1 && this._data.width < 1) {
      if(this.owner._boundsDirty || this._calculatedWidth===0) {
        this._calculatedWidth = isElement(this.owner.parentNode) && Math.round(this.owner.parentNode.contentWidth * this._data.width) || 0
      }
  ...
  protected calculatedHeight() {
    if(this._data.height && this._data.height > -1 && this._data.height < 1){
      if(this.owner._boundsDirty || this._calculatedHeight===0){
        this._calculatedHeight =  isElement(this.owner.parentNode) && Math.round(this.owner.parentNode.contentHeight * this._data.height) || 0
      }
    }
  ...
  //   if (this._data.left && this._data.left > -1 && this._data.left < 1) {
  //     if(this.owner._positionDirty||this.owner ._boundsDirty || this._calculatedLeft===0) {
  //       this._calculatedLeft = isElement(this.owner.parentNode) && Math.round(this.owner.parentNode.contentWidth * this._data.left) || 0
  //     }
  //   } else if( this._calculatedLeft===0&& this._data.left) {
  //     this._calculatedLeft =    this._data.left
  //   }
  ...
  */
  ...
  //  Celse  
   else if (this.owner._positionDirty &&this.owner.parentElement && this._data.top > -1 && this._data.top < 1) {
      // if(this.owner.parentElement && (this.owner._positionDirty || this.owner.parentElement!._boundsDirty || this._calculatedTop===0)) {
        this._calculatedTop =  
        // this.owner.parentElement &&  (
          // this._calculatedTop===0 || 
          
          // this.owner._boundsDirty 
          // this.owner.parentElement!._boundsDirty ||
          //  this.owner.parentElement!._positionDirty 
          //  ) && 
           Math.round( this.owner.parentElement!.contentHeight * this._data.top) ||  this._calculatedTop
      // }
      // this._calculatedTop =this.owner.parentElement&& Math.round( this.owner.parentElement!.contentHeight * this._data.top) || this._data.top
      // debug(this._calculatedTop)
    } 
    else  if(this.owner._positionDirty) {
