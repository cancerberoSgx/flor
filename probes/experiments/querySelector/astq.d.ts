...
declare class ASTQ<Node = any> {
  ...
  constructor()
  /**
   * Register one or more custom tree access adapter(s) to support arbitrary AST-style data structures. 
   *
   * The ASTQAdapter has to conform to a particular duck-typed interface. See below for more information. 
   *
   * By default ASTq has built-in adapters for ASTy, XML DOM, Parse5, Cheerio, JSON and Mozilla AST. All those
   * "taste" the node passed to ASTQ#query and hence are auto-selected. 
   *
   * Calling adapter() causes these to be replaced with a single custom adapter. 
   *
   * Its "tasting" can be disabled with option force set to true. The ASTQ#adapter returns the API itself.
   *
```js
...
```
  ...
  /**
   * Register function named name by providing the callback func which has to return an arbitrary 
   * value and optionally can access the current node with the help of the selected adapter. 
...
```
  ...
  /**
   * Set the upper limit for the internal query cache to [[num]], i.e., up to [[num]] ASTs of parsed queries will be cached. 
   * 
  ...
  /**
   * Compile [[selector]] DSL into an internal query object for subsequent processing by [[execute]]. 
   * If [trace] is `true` the compiling is dumped to the console. Returns the query object.
   */
  compile(selector: string, trace?: boolean): ASTQQuery

  /**
   * Execute the previously compiled query (see compile above) at node. The optional params object can provide parameters 
   * for the {name} query constructs. If trace is true the execution is dumped to the console. Returns an array of zero 
   * or more matching AST nodes
   */
   ...
   * 
   * Use this as the standard query method except you need more control. The optional params object
   * can provide parameters for the {name} query constructs. If trace is true the compiling and execution is
   * dumped to the console. Returns an array of zero or more matching AST nodes.
   */
...
/**
 * For accessing arbitrary AST-style data structures, an adapter has to be provided. By default ASTq has
 * adapters for use with ASTy, XML DOM, Parse5 and Mozilla AST. 
 */
interface ASTQAdapter<Node = any> {
  ...
  /**
   * Return parent node of [[node]]. In case the underlying data structure does not support traversing to
   * parent nodes, throw an exception.
   */
